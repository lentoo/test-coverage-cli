...
function InstrumenterIstanbul(options) {
  ...
  const instrumenter = createInstrumenter({
    ...
    esModules: 'module',
    debug: true,
    coverageGlobalScope: globalConfig.coverageGlobalScope || 'this',
...
module.exports = async function instrumentAllFileSync({
  ...
  if (stats.isDirectory()) {
    inputDir = input;
    const pathStr = path.resolve(input, "**");
    let unixStylePath = pathStr;
    if (isWin) {
      unixStylePath = path.normalize(pathStr).replace(/\\/g, "/");
    }
    const files = globSync(unixStylePath, {
      dot: true,
      nodir: true,
    ...
    const filesToInstrument = globSync(unixStylePath, {
      dot: true,
      nodir: true,
